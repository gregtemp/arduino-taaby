...
void setup(void) {
  ...
  if (!mpu.begin()) {
    ...
    while (1) {
      delay(10);// hang if it doesnt init 
    }
  }
...
void loop() {
  ...
  if (counter % 1000 == 0) {
    ...
    int sense[7];
    sense[0] = (int)(a.acceleration.x * 10.);
    sense[1] = (int)(a.acceleration.y * 10.);
    sense[2] = (int)(a.acceleration.z * 10.);
// taking this out for now, not sure if its working too well
...
//    sense[6] = (int)(temp.temperature*10.);
    if (bstate == 1) {
      ...
      display.println(sense[0]);
      display.print(F("Accel Y:  "));
      display.println(sense[1]);
      display.print(F("Accel Z:  "));
      display.print(sense[2]);
      display.display();
      Serial.println("");
      setRGB(255, 0, 0);
    }
